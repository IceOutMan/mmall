...
//    private static final Logger logger = LoggerFactory.getLogger(OrderServiceImpl.class);
    ...
    private long generateOrderNo() {
        ...
        return currentTiem + new Random().nextInt(100);
    }
    ...
    private ServerResponse<List<OrderItem>> getCartOrderItem(Integer userId, List<Cart> cartList) {
        ...
        for (Cart cartItem : cartList) {
            ...
            if (Const.ProductStatusEnum.ON_SALE.getCode() != product.getStatus()) {//产品不在线
                return ServerResponse.createByErrorMessage("产品" + product.getName() + "不是在线售卖状态");
            }

            if (cartItem.getQuantity() > product.getStock()) {//校验库存
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path) {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                //订单成功后将生成的二维码上传到服务器上去  和上传文件的逻辑基本是一致的
    ...
    public ServerResponse aliCallback(Map<String, String> params) {
        ...
        if (order == null) {
            return ServerResponse.createByErrorMessage("非我的订单,回调忽略");
        }
    ...
    public void closeOrder(int hour) {
        ...
        for (Order order : orderList) {
            ...
            for (OrderItem orderItem : orderItemList) {
                //一定要使用主键 where 条件， 防止锁表。同时必须是支持 MySql的InnoDB引擎
                Integer stock = productMapper.selectStockByProductId(orderItem.getProductId());

                //考虑的已生成的订单里的商品，被删除的情况
                if (stock == null) {
                    continue;
                }
                Product product = new Product();
                product.setId(orderItem.getProductId());
                product.setStock(stock + orderItem.getQuantity());//目前的库存数+要关闭订单的数量
                productMapper.updateByPrimaryKeySelective(product);
            }
            //关闭这个订单
